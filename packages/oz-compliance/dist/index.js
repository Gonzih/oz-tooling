...
server.tool("check_90pct_asset_test", "Check whether a QOF meets the 90% asset test (≥90% of assets must be QOZP — Qualified OZ Property)", {
    total_assets: zod_1.z.number().positive().describe("Total QOF assets in USD"),
    qozp_assets: zod_1.z.number().min(0).describe("Assets qualifying as QOZP (Qualified OZ Property) in USD"),
    measurement_date: zod_1.z.string().optional().describe("Measurement date (YYYY-MM-DD). Defaults to today."),
...
server.tool("check_qozb_eligibility", "Check whether a business qualifies as a QOZB (Qualified Opportunity Zone Business)", {
    gross_income_in_oz_pct: zod_1.z.number().min(0).max(100).describe("Percentage of gross income derived from active conduct in OZ"),
    tangible_property_in_oz_pct: zod_1.z.number().min(0).max(100).describe("Percentage of tangible property located in OZ"),
    nonqualified_financial_pct: zod_1.z.number().min(0).max(100).describe("Percentage of assets that are nonqualified financial property (must be <5%)"),
    sin_business: zod_1.z.boolean().default(false).describe("True if business is a sin business (golf courses, country clubs, massage parlors, hot tub facilities, suntan facilities, racetracks, casinos, liquor stores)"),
...
server.tool("check_substantial_improvement", "Check whether property meets the substantial improvement requirement for QOZP", {
    acquisition_cost: zod_1.z.number().positive().describe("Property acquisition cost in USD"),
    improvements_made: zod_1.z.number().min(0).describe("Amount of improvements made so far in USD"),
    months_elapsed: zod_1.z.number().int().min(0).describe("Months elapsed since acquisition"),
    total_months_allowed: zod_1.z.number().int().min(1).default(30).describe("Total months allowed for improvement (default 30)"),
...
server.tool("get_reporting_requirements", "Get OZ 2.0 annual reporting requirements under IRC §§6039K and 6039L", {
    fund_size_usd: zod_1.z.number().positive().describe("Total QOF assets in USD"),
}, async ({ fund_size_usd }) => {
    const isLarge = fund_size_usd > 10000000;
    const perReturnPenalty = isLarge ? 50000 : 10000;
    const annualItems = [
...
server.tool("calculate_penalty_exposure", "Calculate maximum penalty exposure for QOF reporting failures", {
    fund_size_usd: zod_1.z.number().positive().describe("Total QOF assets in USD"),
    returns_count: zod_1.z.number().int().min(1).describe("Number of investor returns that failed to report"),
    willful: zod_1.z.boolean().default(false).describe("True if noncompliance was willful (enhanced penalties apply)"),
}, async ({ fund_size_usd, returns_count, willful }) => {
    const isLarge = fund_size_usd > 10000000;
    const basePerReturn = isLarge ? 50000 : 10000;
...
server.tool("check_working_capital_safe_harbor", "Check whether a business qualifies for the working capital safe harbor (31-month deployment rule)", {
    cash_amount: zod_1.z.number().min(0).describe("Cash/cash equivalents held by QOZB in USD"),
    written_schedule_exists: zod_1.z.boolean().describe("True if a written deployment schedule exists"),
    deployment_plan_exists: zod_1.z.boolean().describe("True if a written plan for deployment of capital into qualified property exists"),
    months_held: zod_1.z.number().int().min(0).default(0).describe("Months capital has been held (must be ≤31 for safe harbor)"),
