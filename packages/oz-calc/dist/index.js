 ...
 */
function calcOzBenefit(gainAmount, holdYears, isQrof, ozVersion, taxRate) {
    ...
    if (ozVersion === "1.0") {
        ...
        if (holdYears >= 10)
            notes.push("Full appreciation exclusion at 10 years applied");
        ...
        if (holdYears >= 10)
            notes.push("Full appreciation exclusion at ≥10 years applied");
        if (holdYears >= 30)
            notes.push("FMV step-up basis applies at 30 years");
        if (isQrof)
            notes.push("QROF: rural zone (pop <50k) — 30% step-up, 50% property reinvestment threshold");
...
server.tool("calculate_oz_benefit", "Calculate Opportunity Zone tax benefits for a specific investment scenario", {
    gain_amount: zod_1.z.number().positive().describe("Capital gain amount in USD"),
    investment_date: zod_1.z.string().optional().describe("Investment date (YYYY-MM-DD). Defaults to today."),
    hold_years: zod_1.z.number().int().min(0).max(40).describe("Planned hold period in years"),
    is_qrof: zod_1.z.boolean().default(false).describe("True if investing in a Qualified Rural Opportunity Fund (QROF)"),
    oz_version: zod_1.z.enum(["1.0", "2.0"]).default("2.0").describe("OZ version: 1.0 (current, expires) or 2.0 (OBBBA, effective Jan 1 2027)"),
    tax_rate: zod_1.z.number().min(0).max(1).default(DEFAULT_FED_RATE).describe("Marginal tax rate on capital gains (default 0.238 = 23.8% federal LTCG + NIIT)"),
...
server.tool("compare_oz_vs_no_oz", "Side-by-side comparison of OZ investment vs. paying tax immediately", {
    gain_amount: zod_1.z.number().positive().describe("Capital gain amount in USD"),
    marginal_rate: zod_1.z.number().min(0).max(1).default(DEFAULT_FED_RATE).describe("Marginal tax rate (default 0.238)"),
    hold_years: zod_1.z.number().int().min(0).max(40).describe("Hold period in years"),
    is_qrof: zod_1.z.boolean().default(false).describe("True if QROF scenario"),
    assumed_annual_return: zod_1.z.number().min(0).max(1).default(0.08).describe("Assumed annual return on OZ investment (default 0.08 = 8%)"),
...
server.tool("check_180_day_window", "Check if a capital gain is still within the 180-day OZ investment window", {
...
server.tool("oz1_vs_oz2", "Compare OZ 1.0 and OZ 2.0 outcomes for a specific investment", {
    gain_amount: zod_1.z.number().positive().describe("Capital gain amount in USD"),
    investment_year: zod_1.z.number().int().describe("Year of OZ investment (e.g. 2026 for OZ 1.0, 2027+ for OZ 2.0)"),
    hold_years: zod_1.z.number().int().min(1).max(40).describe("Planned hold period in years"),
    tax_rate: zod_1.z.number().min(0).max(1).default(DEFAULT_FED_RATE).describe("Marginal capital gains tax rate"),
