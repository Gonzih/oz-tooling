...
// Representative OZ 1.0 dataset — subset of IRS Notice 2018-48 designated tracts
// Full list: https://www.irs.gov/pub/irs-drop/n-18-48.pdf
// Format: fips_tract -> { state, county, median_income_pct, rural, population }
interface ZoneRecord {
  state: string;
  state_abbr: string;
  county: string;
  median_income_pct: number; // % of state/metro median income
  rural: boolean;
  population: number;
  oz1_designated: boolean;
}

const OZ_ZONES: Record<string, ZoneRecord> = {
  // California
...
function isOz2Eligible(record: ZoneRecord): boolean {
  return record.median_income_pct <= OZ2_INCOME_THRESHOLD;
}

function isQrofEligible(record: ZoneRecord): boolean {
...
server.tool(
  "lookup_zone",
  "Look up OZ 1.0 designation and OZ 2.0 eligibility for a census tract FIPS code",
  {
    fips_census_tract: z.string().describe("11-digit FIPS census tract code (e.g. 06001402100)"),
  },
...
server.tool(
  "check_qrof_eligible",
  "Check if a census tract qualifies for Qualified Rural Opportunity Fund (QROF) treatment",
  {
    fips_census_tract: z.string().describe("11-digit FIPS census tract code"),
  },
  async ({ fips_census_tract }) => {
    const tract = fips_census_tract.replace(/\s/g, "");
    const record = OZ_ZONES[tract];
...
server.tool(
  "get_state_zones",
  "Get all OZ 1.0 designated tracts for a state with OZ 1.0/2.0 status",
  {
    state_abbr: z.string().length(2).describe("Two-letter state abbreviation (e.g. CA, TX, NY)"),
  },
  async ({ state_abbr }) => {
...
server.tool(
  "search_zones_by_county",
  "Search for OZ tracts within a specific county",
  {
    state_abbr: z.string().length(2).describe("Two-letter state abbreviation"),
    county_name: z.string().describe("County name (partial match supported)"),
  },
...
server.tool(
  "get_oz_timeline",
  "Get state-specific OZ nomination timelines and key dates",
  {
    state_abbr: z.string().length(2).optional().describe("Two-letter state abbreviation (optional — omit for federal timeline)"),
  },
  async ({ state_abbr }) => {
