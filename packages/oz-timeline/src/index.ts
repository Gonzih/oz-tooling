...
function urgencyLevel(days: number): "EXPIRED" | "CRITICAL" | "URGENT" | "SOON" | "OK" {
...
interface OZDeadline {
  date: string;
  event: string;
  category: "OZ_1.0" | "OZ_2.0" | "BOTH";
  days_until?: number;
  urgency?: string;
}

const STATIC_DEADLINES: Omit<OZDeadline, "days_until" | "urgency">[] = [
  {
    date: "2026-07-01",
...
server.tool(
  "get_all_deadlines",
  "Get all OZ key dates sorted chronologically with days remaining",
  {},
  async () => {
    const today = new Date();
    const deadlines: OZDeadline[] = STATIC_DEADLINES.map(d => ({
      ...d,
...
server.tool(
  "check_investment_window",
  "Check 180-day OZ investment window for a given gain recognition date",
  {
...
server.tool(
  "days_until_oz2",
  "Calculate days until OZ 2.0 is effective (Jan 1, 2027)",
  {
    reference_date: z.string().optional().describe("Reference date (YYYY-MM-DD). Defaults to today."),
  },
...
server.tool(
  "get_nomination_schedule",
  "Get governor nomination schedule and timelines for OZ 2.0 zone designations",
  {
    state_abbr: z.string().length(2).optional().describe("Two-letter state abbreviation (optional — omit for all states summary)"),
  },
  async ({ state_abbr }) => {
    const today = new Date();
    ...
    const stateSchedules: Record<string, { agency: string; typical_window_days: number; notes: string }> = {
...
server.tool(
  "oz1_deadline_check",
  "Check the OZ 1.0 Dec 31, 2026 gain recognition deadline and urgency",
  {},
  async () => {
    const today = new Date();
